	...
	if(!item_in_hand && isliving(dragged))
		...
		if(stat == CONSCIOUS && can_piggyback(target))
			...
			if(user == target)
				...
				var/response = alert(src, "[user.name] is requesting a piggyback ride.", "Piggyback Ride", list("Yes, let them on", "No"))
				if(response == "No")
			...
			// if the person dragged the user onto themselves, prompt the user
				...
				var/response = alert(target, "[src.name] is offering to give you a piggyback ride.", "Piggyback Ride", list("Yes, get on", "No"))
				if(response == "No")
